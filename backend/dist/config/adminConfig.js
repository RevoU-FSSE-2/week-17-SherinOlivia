...
const insertAdmin = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const [adminCheck] = yield dbConnection_1.DBLocal.promise().query(`SELECT * FROM w17.users WHERE role = 'admin'`);
        if (Object.keys(adminCheck).length === 0) {
            const adminUsername = process.env.ADMIN_USERNAME;
            const adminEmail = process.env.ADMIN_EMAIL;
            const adminPass = process.env.ADMIN_PASS;
            const hashedPass = yield bcrypt_1.default.hash(adminPass, 10);
            yield dbConnection_1.DBLocal.promise().query(`INSERT INTO w17.users (username, email, password, role) VALUES ('${adminUsername}','${adminEmail}', '${hashedPass}', 'admin')`);
