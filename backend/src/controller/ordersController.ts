...
const createNewOrder = async (req: Request, res: Response) => {
    try {
        ...
        if (role == "staff" || role == "admin") {
            ...
            const [newOrder] = await DB.promise().query(`INSERT INTO w17.orders (custId, product_name, order_qty, total, status, order_datetime, isDeleted)
            ...
            const [createdOrder] = await DB.promise().query(`SELECT * FROM w17.orders WHERE id = ?`,
...
const updateOrderStatus = async (req: Request, res: Response) => {
    ...
    try {
        const getOrder = await DB.promise().query(`SELECT * FROM w17.orders WHERE id = ? AND isDeleted = ?`,
        ...
        if (getOrder[0].length > 0) {
            ...
            return
                ...
                await DB.promise().query(`UPDATE w17.orders SET status = ? WHERE id = ?`,
                [status, id])
    
                const updatedOrder = await DB.promise().query(`SELECT * FROM w17.orders WHERE id = ?`,
                [id]) as RowDataPacket[]
